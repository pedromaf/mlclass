...
def Reward(state0, state1, fruit_reward):
	...
	goal_progress = int((state0_goal_distance/state1_goal_distance))
...
for f in range(nb_frames):
    ...
    current_Q = int(agent.q_table[state0.get("player_x")][state0.get("fruit_x")][action_index])
    agent.q_table[state0.get("player_x")][state0.get("fruit_x")][action] = current_Q + alpha*(reward + gamma*(agent.maxQAction(state1)) - current_Q)
